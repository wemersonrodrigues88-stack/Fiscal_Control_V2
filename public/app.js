...
const isControl=()=>CONTROL.includes(state.user?.profile);const load=async()=>{const d=await api('/api/apuracoes');d.stores=Array.isArray(d.stores)?d.stores:[];d.items=Array.isArray(d.items)?d.items:(Array.isArray(d.executions)?d.executions:[]);d.checklist=Array.isArray(d.checklist)?d.checklist:[];state.data=d;return d};
...
const manager=(c,d)=>{const as=[...new Set(d.stores.map(s=>s.analyst).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR')),states=[...new Set(d.stores.map(s=>s.state).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR')),canChrono=['Gestão','Coordenador','Desenvolvedor'].includes(state.user?.profile);const timer=x=>{if(!canChrono)return '';if(x.flow_phase==='Gerando')return '<small class="timer" data-t-start="'+esc(x.started_at||'')+'" data-t-query="'+esc(x.query_generated_at||'')+'">'+secs(elapsed(x.started_at,x.query_generated_at||null))+'</small>';if(x.flow_phase==='Analisando')return '<small class="timer" data-t="'+esc(x.flow_analyzing_at||x.analyzing_at||'')+'">'+secs(elapsed(x.flow_analyzing_at||x.analyzing_at))+'</small>';if(x.flow_phase==='Finalizado'||x.status==='Finalizado')return '<small class="timer">Total '+secs(elapsed(x.started_at,x.query_generated_at)+elapsed(x.flow_analyzing_at||x.analyzing_at,x.finished_at))+'</small>';return ''};c.innerHTML='<div class="section-title"><div><h2>Apurações — Acompanhamento</h2><p class="muted">Visão geral individual, relatórios e correção de status.</p></div><div class="ap-toolbar"><button class="secondary" id="ap-refresh">Atualizar</button><button class="primary" id="ap-report-partial">Relatório</button><button class="primary" id="abc-report">Curva ABC</button></div></div><div class="ap-filters"><select id="ap-analyst"><option value="">Todos os analistas</option>'+as.map(a=>'<option>'+esc(a)+'</option>').join('')+'</select><select id="ap-store"><option value="">Todas as lojas</option>'+d.stores.map(s=>'<option value="'+s.id+'">'+esc(s.number)+' · '+esc(s.name)+'</option>').join('')+'</select><select id="ap-state"><option value="">Todos os estados</option>'+states.map(st=>'<option>'+esc(st)+'</option>').join('')+'</select></div><div class="card"><div class="table-wrap"><table class="ap-table"><thead><tr><th>Loja</th><th>Analista</th>'+TAXES.map(t=>'<th>'+t+'</th>').join('')+'</tr></thead><tbody>'+d.stores.map(s=>'<tr data-a="'+esc(s.analyst||'')+'" data-s="'+s.id+'" data-st="'+esc(s.state||'')+'"><td>'+managerStoreControl(s,d)+'</td><td>'+esc(s.analyst||'Sem carteira')+'</td>'+TAXES.map(t=>{const x=d.items.find(i=>i.store_id===s.id&&i.obligation===t)||{store_id:s.id,obligation:t,status:'Pendente'};return '<td><span class="badge '+cls(x.status)+'">'+x.status+'</span>'+timer(x)+rollbackButton(x)+'</td>'}).join('')+'</tr>').join('')+'</tbody></table></div></div><div class="card"><h3>Resumo individual</h3><div id="ap-individual">Selecione um analista ou uma loja.</div></div>';c.querySelector('#ap-analyst').onchange=()=>filter(c,d);c.querySelector('#ap-store').onchange=()=>filter(c,d);c.querySelector('#ap-state').onchange=()=>filter(c,d);c.querySelector('#ap-refresh').onclick=async()=>{c.querySelector('#ap-refresh').disabled=true;try{render(c,await load())}finally{c.querySelector('#ap-refresh')?.removeAttribute('disabled')}};c.querySelector('#ap-report-partial').onclick=()=>openReportSelector();c.querySelector('#abc-report').onclick=abc;c.querySelectorAll('.ap-correct').forEach(b=>b.onclick=()=>rollback(c,b.dataset.id,b.dataset.tax,b.dataset.status));bind(c);clearInterval(tick);tick=setInterval(()=>c.querySelectorAll('.timer').forEach(x=>{if(x.dataset.tStart)x.textContent=secs(elapsed(x.dataset.tStart,x.dataset.tQuery||null));else if(x.dataset.t)x.textContent=secs(elapsed(x.dataset.t))}),1000)};let tick;async function rollback(c,id,tax,current){const options=current==='Finalizado'?['Analisando','Gerando','Pendente']:current==='Analisando'?['Gerando','Pendente']:['Pendente'];const choice=prompt('Corrigir '+tax+'\nStatus atual: '+current+'\nDigite o novo status:\n'+options.map((x,i)=>(i+1)+' - '+x).join('\n'),'1');const n=Number(choice);if(!options[n-1])return;const target=options[n-1];if(!confirm('Voltar '+tax+' de '+current+' para '+target+'?'))return;try{await api('/api/apuracoes/rollback',{method:'PUT',body:JSON.stringify({store_id:Number(id),obligation:tax,status:target})});render(c,await load())}catch(e){alert(e.message)}}
async function check(c,id){
  ...
  const renderChecklist=(rows=[])=>{
    ...
    const opt=k=>k==='quebra_sequencia'?'<button data-v="feito">Sem quebra 💚</button><button data-v="ha_quebras">Há quebra 🟡</button>':k==='curva_abc'?'<button data-v="feito">Sem Diferença</button><button data-v="incons_comercial">Diferença Comercial</button><button data-v="incons_contabil">Diferença Contábil</button>':'<button data-v="feito">Feito 💚</button>';
    ...
    c.querySelectorAll('.check-options button').forEach(b=>b.onclick=async()=>{
      ...
      try{
        await api('/api/icms-checklist',{method:'PUT',body:JSON.stringify({store_id:id,item_key:w.dataset.item,status:b.dataset.v})});
        w.querySelectorAll('button').forEach(x=>x.classList.remove('selected'));
        b.classList.add('selected');
        w.previousElementSibling.querySelector('small').textContent='Concluído';
      }catch(e){alert(e.message)}
    });
    c.querySelector('#back-ap').onclick=()=>load().then(d=>render(c,d));
  };
  renderChecklist([]);
  try{
    const d=await api('/api/icms-checklist');
    renderChecklist(Array.isArray(d.data)?d.data:[]);
  }catch(e){
    const box=c.querySelector('.checklist');
    if(box){const err=document.createElement('div');err.className='error';err.textContent='Não foi possível carregar os resultados salvos do checklist. Os itens continuam disponíveis para preenchimento.';box.prepend(err);}
  }
}
