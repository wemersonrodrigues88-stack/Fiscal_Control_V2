...
const isControl=()=>CONTROL.includes(state.user?.profile);const load=async()=>{const d=await api('/api/state');state.data=d;return d};
...
const debtorStatus=(s,d)=>d.executions.find(i=>i.store_id===s.id&&i.obligation==='ICMS')?.icms_debtor_status||null;
