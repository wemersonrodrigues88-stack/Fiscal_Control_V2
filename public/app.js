...
const action=x=>{const phase=x.flow_phase||null;if(x.status==='Pendente'){const gerar='<button class="badge blue ap-action" data-id="'+(x.store_id||x.id)+'" data-tax="'+esc(x.obligation)+'" data-next="Gerando">Gerar</button>';if(x.obligation==='ICMS'){if(x.icms_debtor_status==='aguardando_transferencia')return gerar+'<span class="badge yellow icms-debtor-status">Loja devedora, aguardando transferência de crédito</span>';if(x.icms_debtor_status==='transferencia_aprovada')return gerar+'<span class="badge green icms-debtor-status">Transferência de crédito aprovada</span>';if(x.icms_debtor_status==='finalizada_devedora')return gerar+'<span class="badge green icms-debtor-status">Finalizar como devedora</span>';return gerar+'<button class="badge yellow ap-debtor" data-id="'+(x.store_id||x.id)+'" data-tax="ICMS">Loja devedora, aguardando transferência de crédito</button>';}return gerar;}if(phase==='Gerando'||phase==='Query')return '<button class="badge blue ap-action" data-id="'+(x.store_id||x.id)+'" data-tax="'+esc(x.obligation)+'" data-next="Analisando">Analisar</button>';if(phase==='Analisando'||(!phase&&x.status==='Analisando'))return '<button class="badge green ap-action" data-id="'+(x.store_id||x.id)+'" data-tax="'+esc(x.obligation)+'" data-next="Finalizado">Finalizar</button>';return '<span class="badge green">Finalizada</span>';};
...
const bind=c=>{c.querySelectorAll('.ap-action').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await api('/api/apuracoes/status',{method:'PUT',body:JSON.stringify({store_id:Number(b.dataset.id),obligation:b.dataset.tax,status:b.dataset.next})});render(c,await load())}catch(e){alert(e.message);b.disabled=false}});c.querySelectorAll('.ap-debtor').forEach(b=>b.onclick=async()=>{if(!confirm('Registrar esta loja como devedora e solicitar transferência de crédito?'))return;b.disabled=true;try{await api('/api/icms-debtor',{method:'PUT',body:JSON.stringify({store_id:Number(b.dataset.id),action:'request'})});render(c,await load())}catch(e){alert(e.message);b.disabled=false}});c.querySelectorAll('.icms-check').forEach(b=>b.onclick=()=>check(c,Number(b.dataset.store)));c.querySelectorAll('.ap-adjust-time').forEach(b=>b.onclick=()=>adjustTime(b));c.querySelectorAll('.icms-debtor-note').forEach(b=>b.onclick=()=>openDebtorDecision(c,Number(b.dataset.id),b.dataset.name));};
...
async function openDebtorDecision(c,id,name){
  const old=document.getElementById('icms-debtor-modal');if(old)old.remove();
  const m=document.createElement('div');m.id='icms-debtor-modal';m.style.cssText='position:fixed;inset:0;background:rgba(15,23,42,.42);display:flex;align-items:center;justify-content:center;padding:20px;z-index:9999';
