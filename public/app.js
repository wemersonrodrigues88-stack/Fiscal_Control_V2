...
const managerStoreControl=(s,d)=>{const st=debtorStatus(s,d);const label=st==='aguardando_transferencia'?'🟡 Loja devedora — aguardando transferência de crédito':st==='transferencia_aprovada'?'🟢 Transferência de crédito aprovada':st==='finalizada_devedora'?'🟢 Finalizar como devedora':st==='solicitacao_indev'?'Solicitação indevida':'Normal';return '<div class="store-name-control"><button type="button" class="store-status-trigger" data-id="'+s.id+'"><b>'+esc(s.number)+' · '+esc(s.name)+'</b><small>'+esc(s.state||'')+'</small></button><div class="store-status-menu"><div class="store-status-title">Situação do ICMS</div><div class="store-status-current">'+label+'</div>'+(st==='aguardando_transferencia'?'<button type="button" class="store-status-option" data-id="'+s.id+'" data-action="approve">Transferência aprovada</button><button type="button" class="store-status-option" data-id="'+s.id+'" data-action="finalize_debtor">Finalizar como devedora</button><button type="button" class="store-status-option" data-id="'+s.id+'" data-action="invalid_request">Solicitação indevida</button>':'')+'</div></div>'};
...
const analyst=(c,d)=>{c.innerHTML=`<div class="section-title"><div><p class="muted">Somente as lojas da sua carteira.</p></div></div><div class="ap-grid">${d.stores.map(s=>`<article class="card"><div class="ap-store-head">${analystStoreControl(s,d)}<button class="primary small icms-check" data-store="${s.id}">Checklist ICMS</button></div>${TAXES.map(t=>{const x=d.items.find(i=>i.store_id===s.id&&i.obligation===t)||{store_id:s.id,obligation:t,status:'Pendente'};return `<div class="ap-row"><div><b>${t}</b><small>Status: ${x.status}</small></div>${action(x)}</div>`}).join('')}</article>`).join('')}</div>`;bind(c)};
