(function(){
  ...
  const isManager=()=>['Gestão','Desenvolvedor'].includes(window.state?.user?.profile);
  async function load(){const r=await window.api('/api/deadline-configs');cache=r.data||[];return r}
  function dateFor(t,s){return fmt(cache.find(x=>x.obligation===t&&x.state===s)?.due_date)}
  function render(c,r){
    const manager=isManager();
    const stores=(window.state.data?.stores||[]);
